#!/usr/bin/env bats

MODE=${MODE:-"ids"}

setup() {
    if [ "$(uname)" = "Darwin" ]; then
        # New installation uses /opt/suricata instead of Homebrew paths
        export BIN_FOLDER="/opt/suricata"
        export LOG_DIR="/var/log/suricata"
        export CONFIG_DIR="/etc/suricata"
        export CONFIG_FILE="$CONFIG_DIR/suricata.yaml"
        export RULES_DIR="/var/lib/suricata/rules"
    else
        export LOG_DIR="/var/log/suricata"
        export CONFIG_DIR="/etc/suricata"
        export CONFIG_FILE="$CONFIG_DIR/suricata.yaml"
        export RULES_DIR="/var/lib/suricata/rules"
    fi
}

@test "YQ is installed" {
    run command -v yq
    [ "$status" -eq 0 ]
}

@test "Suricata is installed" {
    run command -v suricata
    [ "$status" -eq 0 ]
}

@test "Suricata-update is accessible" {
    run command -v suricata-update
    [ "$status" -eq 0 ]
}

@test "Suricata-update wrapper works on ARM (macOS only)" {
    if [ "$(uname)" = "Darwin" ] && [ "$(uname -m)" = "arm64" ]; then
        # Check if wrapper script exists
        [ -f "/usr/local/bin/suricata-update" ]
        # Check if it's a bash script (wrapper) not a symlink
        run head -1 /usr/local/bin/suricata-update
        [[ "$output" == "#!/bin/bash" ]]
    else
        skip "This test is specific to macOS ARM64"
    fi
}

@test "Suricata-update symlink exists on Intel (macOS only)" {
    if [ "$(uname)" = "Darwin" ] && [ "$(uname -m)" = "x86_64" ]; then
        # Check if symlink exists
        [ -L "/usr/local/bin/suricata-update" ]
        # Verify it points to the right location
        run readlink /usr/local/bin/suricata-update
        [[ "$output" == "/opt/suricata/bin/suricata-update" ]]
    else
        skip "This test is specific to macOS Intel"
    fi
}

@test "Rules file exists" {
    echo "Looking for rules in: $RULES_DIR"
    run sudo ls "$RULES_DIR"
    [ "$status" -eq 0 ]
    run sudo test -f "$RULES_DIR/suricata.rules"
    [ "$status" -eq 0 ] || skip "suricata.rules not found in expected path"
}

@test "Configuration file exists" {
    echo "Checking configuration file: $CONFIG_FILE"
    run sudo test -f "$CONFIG_FILE"
    [ "$status" -eq 0 ]
}

@test "Suricata service is running (Linux only)" {
    if [ "$(uname)" = "Linux" ]; then
        run sudo systemctl is-active suricata
        [ "$status" -eq 0 ]
        [ "$output" = "active" ]
    else
        skip "This test is Linux-specific"
    fi
}

@test "Suricata process is running (macOS only)" {
    if [ "$(uname)" = "Darwin" ]; then
        if ! command -v pgrep >/dev/null; then
            skip "pgrep is not installed"
        fi
        run sudo pgrep suricata
        [ "$status" -eq 0 ]
    else
        skip "This test is macOS-specific"
    fi
}

@test "Community ID is enabled in configuration" {
    run sudo grep -q "community-id: true" "$CONFIG_FILE"
    [ "$status" -eq 0 ]
}

@test "Detect-engine configuration is present" {
    run sudo grep -q "detect-engine:" "$CONFIG_FILE"
    [ "$status" -eq 0 ]
}

@test "Eve-log types include 'alert'" {
    run sudo yq eval '.outputs[] | select(has("eve-log")) | .["eve-log"].types[]' "$CONFIG_FILE"
    echo "$output"
    [ "$status" -eq 0 ]
    [[ "$output" == *"alert"* ]]
}

@test "Drop.conf is created in IPS mode" {
    if [ "$MODE" = "ips" ]; then
        run sudo test -f "$CONFIG_DIR/drop.conf"
        [ "$status" -eq 0 ]
    else
        skip "This test is specific to IPS mode."
    fi
}

@test "Drop.conf has necessary config" {
    if [ "$MODE" = "ips" ]; then
        run sudo grep -q "group:emerging-attack_response" "$CONFIG_DIR/drop.conf"
        [ "$status" -eq 0 ]
    else
        skip "This test is specific to IPS mode."
    fi
}

@test "LISTENMODE is set to nfqueue in IPS mode" {
    if [ "$MODE" = "ips" ]; then
        run sudo test -f "/etc/default/suricata"
        [ "$status" -eq 0 ] || skip "/etc/default/suricata not found"
        run sudo grep -q "LISTENMODE=nfqueue" /etc/default/suricata
        [ "$status" -eq 0 ]
    else
        skip "This test is specific to IPS mode."
    fi
}

@test "DEFAULT_INPUT_POLICY is set to ACCEPT in IPS mode" {
    if [ "$MODE" = "ips" ]; then
        run sudo test -f "/etc/default/ufw"
        [ "$status" -eq 0 ] || skip "/etc/default/ufw not found"
        run sudo grep -q "DEFAULT_INPUT_POLICY=\"ACCEPT\"" /etc/default/ufw
        [ "$status" -eq 0 ]
    else
        skip "This test is specific to IPS mode."
    fi
}

@test "NFQUEUE rules are added to UFW in IPS mode" {
    if [ "$MODE" = "ips" ]; then
        run sudo test -f "/etc/ufw/before.rules"
        [ "$status" -eq 0 ] || skip "/etc/ufw/before.rules not found"
        run sudo grep -q "^-I INPUT -j NFQUEUE" /etc/ufw/before.rules
        [ "$status" -eq 0 ]
        run sudo grep -q "^-I OUTPUT -j NFQUEUE" /etc/ufw/before.rules
        [ "$status" -eq 0 ]
    else
        skip "This test is specific to IPS mode."
    fi
}

@test "Custom drop rule is present in $RULES_DIR/suricata.rules in IPS mode" {
    run sudo test -f "$RULES_DIR/suricata.rules"
    [ "$status" -eq 0 ] || skip "suricata.rules not found in expected path"
    run sudo grep -q "sid:992002087" "$RULES_DIR/suricata.rules"
    [ "$status" -eq 0 ]
}

# Tests for new Ubuntu prebuilt binary functionality
@test "Ubuntu version detection works (Ubuntu only)" {
    if [ "$(uname)" = "Linux" ]; then
        if [ -f /etc/os-release ]; then
            . /etc/os-release
            if [ "$ID" = "ubuntu" ]; then
                case "$VERSION_ID" in
                    "22.04"|"24.04")
                        echo "Detected supported Ubuntu version: $VERSION_ID"
                        [ "$VERSION_ID" = "22.04" ] || [ "$VERSION_ID" = "24.04" ]
                        ;;
                    *)
                        skip "Unsupported Ubuntu version: $VERSION_ID"
                        ;;
                esac
            else
                skip "Not running on Ubuntu"
            fi
        else
            skip "/etc/os-release not found"
        fi
    else
        skip "This test is Linux-specific"
    fi
}

@test "Suricata binary exists at expected location (Linux only)" {
    if [ "$(uname)" = "Linux" ]; then
        run test -f "/usr/bin/suricata"
        [ "$status" -eq 0 ]
        run test -L "/usr/bin/suricata"
        # Should pass if it's a symlink (prebuilt) or regular file (PPA)
        [ "$status" -eq 0 ] || [ -f "/usr/bin/suricata" ]
    else
        skip "This test is Linux-specific"
    fi
}

@test "Suricata files exist in /opt for prebuilt installations (Linux only)" {
    if [ "$(uname)" = "Linux" ]; then
        # Check if this is a prebuilt installation by looking for /opt/suricata
        if [ -d "/opt/suricata" ]; then
            echo "Detected prebuilt installation"
            run test -f "/opt/suricata/bin/suricata"
            [ "$status" -eq 0 ]
            run test -L "/usr/bin/suricata"
            [ "$status" -eq 0 ]
            # Verify symlink points to /opt/suricata/bin/suricata
            run readlink /usr/bin/suricata
            [[ "$output" == "/opt/suricata/bin/suricata" ]]
        else
            skip "Not a prebuilt installation (/opt/suricata not found)"
        fi
    else
        skip "This test is Linux-specific"
    fi
}

@test "Systemd service file exists for prebuilt installations (Linux only)" {
    if [ "$(uname)" = "Linux" ]; then
        # Check if this is a prebuilt installation
        if [ -d "/opt/suricata" ]; then
            echo "Testing systemd service for prebuilt installation"
            run test -f "/etc/systemd/system/suricata.service"
            [ "$status" -eq 0 ]
            # Verify service file contains correct ExecStart
            run sudo grep -q "ExecStart=/usr/bin/suricata" /etc/systemd/system/suricata.service
            [ "$status" -eq 0 ]
        else
            skip "Not a prebuilt installation"
        fi
    else
        skip "This test is Linux-specific"
    fi
}

@test "Suricata-update symlink exists for prebuilt installations (Linux only)" {
    if [ "$(uname)" = "Linux" ]; then
        # Only test if prebuilt installation and suricata-update exists
        if [ -d "/opt/suricata" ] && [ -f "/opt/suricata/bin/suricata-update" ]; then
            echo "Testing suricata-update symlink for prebuilt installation"
            run test -L "/usr/bin/suricata-update"
            [ "$status" -eq 0 ]
            # Verify symlink points to /opt/suricata/bin/suricata-update
            run readlink /usr/bin/suricata-update
            [[ "$output" == "/opt/suricata/bin/suricata-update" ]]
        else
            skip "Not a prebuilt installation or suricata-update not included"
        fi
    else
        skip "This test is Linux-specific"
    fi
}