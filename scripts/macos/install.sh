...
MODE="ids"
INTERFACE=""
WAZUH_SURICATA_REPO_REF=${WAZUH_SURICATA_REPO_REF:-"v0.2.2"}
...
run_uninstall_script() {
    ...
    download_and_verify_file "$UNINSTALL_MODERN_URL" "$download_path" "scripts/macos/uninstall.sh" "suricata uninstall script" "$WAZUH_SURICATA_REPO_URL/checksums.sha256"
...
download_rules() {
    ...
    download_file "$rules_url" "$rules_archive" || {
        rm -rf "$temp_dir"
...
setup_suricata_config() {
    ...
    if ! maybe_sudo test -f "$CONFIG_FILE"; then
        ...
        if [ -n "$default_config" ]; then
            ...
            if download_file "$FALLBACK_CONFIG_URL" "$CONFIG_FILE"; then
...
main() {
    ...
    if [ "$OS" = "darwin" ] && [ "$(detect_architecture)" = "amd64" ]; then
        ...
        maybe_sudo chmod +x "$remote_installer"
        if ! maybe_sudo bash "$remote_installer" "$@"; then
            error_message "Remote installer failed"
            exit 1
        fi
        exit 0
    fi
